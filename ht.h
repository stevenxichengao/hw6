...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    HashItem* newP = new HashItem(p);
		//currentAlpha = size /  capacitie..
		//std::cout <<"this is the current stuff " << currentAlpha << " " << resizeAlpha_ << std::endl;
    if(currentAlpha >= resizeAlpha_) {
			//std::cout <<"calling reseize " << std::endl;
			resize();
		}
...
{
    mIndex_++;
    if(mIndex_>=28) throw std::logic_error("No more available capacity");
		currentAlpha = 0;
		nDeleted = 0;
    //this->m_ = CAPACITIES[mIndex_];
    std::vector<HashItem*> temptable;
		for(HASH_INDEX_T i = 0; i < CAPACITIES[mIndex_]; i++){
			temptable.push_back(nullptr);
		}

		std::vector<HashItem*> tempTable2;
		tempTable2 = table_;
		table_ = temptable;
		size_ = 0;
    for(HASH_INDEX_T i = 0; i < CAPACITIES[mIndex_-1]; i++){
			//std::cout <<"in for loop " << i << std::endl;
        if(tempTable2[i]!=nullptr) {
            if(tempTable2[i]->deleted!=true) {
							this->insert(tempTable2[i]->item);
